...
# - "string": Use Llama3 for individual probabilities, OpenAI for P(B|X,A)
#
# Allocation methods:
...
#
# Other smoothing: (1) constrain the allocation mechanism -- not to repeat until every value of b is chose at least once. Will do the most probable ones first, and do the balance after it tries all the values. 
#
# equally weighted average (of absolute difference of logs). instead of KL divergence. Undefined if p and q are both 0. 
#
# Or smomething in between: flatten the distribution, then use that to weight the average. 
#
# Prompt phrasing. 
#
# Nested meieutic prompting. After proposing X, give further X' that further DECREASES/INCREASES the target b, or increase some other b.
#
# Average the part of the grey part, then average. So we can see the trend of the part that is more of interest. 
#
# Try a model that is not instruction-tuned, Qwen, Gemma. 
#
##############################################################################################################
...
class MaieuticPromptingMIS:
    def __init__(self, config_path: str, log_path: str, kl_path: str, model_id: str, client, weights_path: str = None, ess_path: str = None):
        ...
        # Initialize small model if in string or verbal_small mode
        self.small_model = None
        self.small_tokenizer = None
        if self.mode in ['string', 'verbal_small']:
    ...
    def _initialize_small_model(self):
        """Initialize the small model (Llama3) for token probability computation and text generation."""
        try:
            ...
            # Store scenario generation temperature for HuggingFace use
            self.scenario_temp = self.config['model_settings']['scenario_generation_temperature']
    ...
    def generate_scenario_with_small_model(self, a_facts: List[str], b_fact: str) -> str:
        """Generate a scenario using the small HuggingFace model."""
        if self.small_model is None:
            raise RuntimeError("Small model not initialized")
        
        # Build prompt from the same config template used in OpenAI version
        scenario_prompt = self.config['prompts']['scenario_generation']
        
        # Convert role-content pairs to a single prompt string
        prompt_text = ""
        ...
        try:
            # Tokenize the prompt
            inputs = self.small_tokenizer(prompt_text, return_tensors="pt", truncation=True, max_length=2048)
            ...
            with torch.no_grad():
                outputs = self.small_model.generate(
                    inputs.input_ids,
                    max_new_tokens=200,  # Reasonable length for scenarios
                    temperature=self.scenario_temp,  # Use same temperature as OpenAI version
                    do_sample=True,
                    pad_token_id=self.small_tokenizer.eos_token_id,
                    eos_token_id=self.small_tokenizer.eos_token_id
                )
            
            # Decode only the generated part (exclude input prompt)
            generated_tokens = outputs[0][inputs.input_ids.shape[1]:]
            generated_text = self.small_tokenizer.decode(generated_tokens, skip_special_tokens=True)
            
            # Log the generation
            with open(self.log_pth, "a") as log_file:
                log_file.write(f"[HF SCENARIO GENERATION]\n")
                log_file.write(f"[PROMPT]: {prompt_text}\n")
                log_file.write(f"[RESPONSE]: {generated_text.strip()}\n")
                log_file.write("-" * 80 + "\n\n")
            ...
            with open(self.log_pth, "a") as log_file:
                log_file.write(f"[ERROR] HuggingFace scenario generation failed: {e}\n")
            raise RuntimeError(f"HuggingFace scenario generation failed: {e}")
    ...
    def get_likelihood_ratio_with_small_model(self, a_facts: List[str], x_scenario: str, b_fact: str) -> float:
        """Get likelihood ratio P(X|A,B)/P(X|A) using the small HuggingFace model."""
        if self.small_model is None:
            raise RuntimeError("Small model not initialized")
        
        try:
            # Build prompts from the same config template used in OpenAI version
            ...
            with torch.no_grad():
                outputs = self.small_model.generate(
                    inputs.input_ids,
                    max_new_tokens=50,  # Shorter response for likelihood ratios
            ...
            generated_tokens = outputs[0][inputs.input_ids.shape[1]:]
            response = self.small_tokenizer.decode(generated_tokens, skip_special_tokens=True)
            ...
            # Log the generation
            with open(self.log_pth, "a") as log_file:
                log_file.write(f"[HF LIKELIHOOD RATIO]\n")
                log_file.write(f"[PROMPT]: {prompt_text}\n")
            ...
            with open(self.log_pth, "a") as log_file:
                log_file.write(f"[ERROR] HuggingFace likelihood ratio failed: {e}\n")
    ...
    def get_probabilities_string(self, a_facts: List[str], x_scenario: str, b_facts: List[str]) -> Tuple[float, List[float]]:
        """Get P(X|A) and P(X|A,B=b_k) for all k using the small model."""
        # Get P(X|A)
    ...
    def process_line(self, a_input: str, b_question: str, b_domain: List[str], ground_truth: Optional[List[float]] = None) -> Dict[str, float]:
        ...
        for i in range(self.total_scenarios):
            ...
            if self.mode in ['verbal', 'verbal_small']:
                ...
                # Store scenario data for string mode with actual probabilities
                scenarios_data.append({
                    "scenario": scenario,
                    "b_value_used": b_value,
                    "b_index_used": option_index,
                    "prob_x_given_a": prob_x_given_a,  # Store for string mode
                    "prob_x_given_a_and_bk": prob_x_given_a_and_bk,  # Store for string mode
                    "likelihood_ratios": likelihood_ratios,  # Keep for logging
                    "p_b_given_scenario": None  # Will be filled later
                })
            ...
            for j, data in enumerate(scenarios_data):
                if self.mode in ['verbal', 'verbal_small']:
                    ...
                    # String mode: weight = P(X|A) / Σₖ(nₖ * P(X|A,B=bₖ))
...
def main():
    ...
    config_folder = "/home/rliu79/reasoning/experiment_maieutic_compare"
    input_folder = "/home/rliu79/reasoning/experiment_maieutic_compare" 
    output_folder = "/home/rliu79/reasoning/experiment_maieutic_compare/output_string_scaled"
    
    # Define all file paths
    config_path = os.path.join(config_folder, "maieutic_config.json")
    input_path = os.path.join(input_folder, "input.txt")
    
    # All output files go to output folder
    ...
    # Additional KL output files will be automatically created in the same output folder
