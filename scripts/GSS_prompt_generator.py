# Maieutic Prompting - Non-Binary Version (Revised)
#
...
class MaieuticPrompting:
    ...
    def compute_kl_divergence(self, predicted: List[float], ground_truth: List[float]) -> float:
        """Compute KL divergence KL(ground_truth || predicted)."""
        epsilon = 1e-10  # Small value to avoid log(0)
        kl = 0.0
        
        for p_true, p_pred in zip(ground_truth, predicted):
            # Add epsilon to avoid log(0)
            p_true = max(p_true, epsilon)
            p_pred = max(p_pred, epsilon)
            kl += p_true * math.log(p_true / p_pred)
    ...
    def process_line(self, a_input: str, b_question: str, b_domain: List[str], ground_truth: Optional[List[float]] = None) -> Dict[str, float]:
        ...
        if ground_truth is not None:
            with open(self.kl_pth, "a") as kl_file:
                kl_str = ", ".join([f"{kl:.6f}" for kl in kl_values])
                kl_file.write(f"{kl_str}\n")
        
        return dict(zip(b_domain, weighted_probs))


def parse_input_line(line: str) -> Tuple[str, str, str, List[str], Optional[List[float]]]:
    """Parse input line of format {B: [question] [domain]; A: [question] [answer]}; [ground_truth]"""
    # Match the pattern with optional ground truth at the end
    # Updated pattern to properly separate the three components
    pattern = r'\{B:\s*([^[]+)\s*\[([^\]]+)\]\s*;\s*A:\s*([^}]+)\}\s*;\s*\[([^\]]+)\]'
    match = re.match(pattern, line.strip())
    
    if match:
        b_question = match.group(1).strip()
        b_domain_str = match.group(2).strip()
        a_part = match.group(3).strip()
        ground_truth_str = match.group(4).strip()
        ...
        if ground_truth_str:
            try:
                # Split by semicolon or comma and convert to floats
                gt_parts = re.split(r'[;,]', ground_truth_str.strip())
                ground_truth = [float(part.strip()) for part in gt_parts if part.strip()]
                
                # Validate that ground truth matches domain size
                if len(ground_truth) != len(b_domain):
                    raise ValueError(f"Ground truth length ({len(ground_truth)}) doesn't match domain size ({len(b_domain)})")
                ...
                raise ValueError(f"Could not parse ground truth: {ground_truth_str}, error: {e}")
        
        # Split A part into question and answer (answer is the last "word"/phrase)
        # Find the last occurrence of a question mark to separate question from answer
        question_end = a_part.rfind('?')
        if question_end != -1:
            a_question = a_part[:question_end + 1].strip()
            a_answer = a_part[question_end + 1:].strip()
        else:
            # Fallback: assume last word/phrase is the answer
            parts = a_part.rsplit(' ', 1)
            if len(parts) == 2:
                a_question, a_answer = parts
            else:
                raise ValueError(f"Could not parse A question and answer from: {a_part}")
        
        return a_question, a_answer, b_question, b_domain, ground_truth
    else:
        # Try fallback pattern without ground truth for backward compatibility
        pattern_no_gt = r'\{B:\s*([^[]+)\s*\[([^\]]+)\]\s*;\s*A:\s*([^}]+)\}'
        match_no_gt = re.match(pattern_no_gt, line.strip())
        
        if match_no_gt:
            b_question = match_no_gt.group(1).strip()
            b_domain_str = match_no_gt.group(2).strip()
            a_part = match_no_gt.group(3).strip()
            
            # Parse B domain
            b_domain = [item.strip() for item in b_domain_str.split(';')]
            
            # Split A part into question and answer
            question_end = a_part.rfind('?')
            if question_end != -1:
                a_question = a_part[:question_end + 1].strip()
                a_answer = a_part[question_end + 1:].strip()
            ...
            return a_question, a_answer, b_question, b_domain, None
        else:
...
def main():
    ...
    input_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/experiment_maieutic_nonbinary/input.txt"
    output_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/experiment_maieutic_nonbinary/output.txt"
    log_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/experiment_maieutic_nonbinary/log.txt"
    kl_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/experiment_maieutic_nonbinary/kl_output.txt"
