...
class AgentS1():
    def __init__(self, config, input, log_pth='./example_files'):
        ...
        if self.structured:
            self.s_root = self.init_root_from_txt(input)
    ...
    def init_root_from_txt(self, input):
        ...
        label_description = self.find_best_matching_label("0", chosen_line , "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/GSS2022-varbook.txt")
    ...
    def s_agent_entry(self, max_step = 100, allow_human = False):      
        ...
        if step_mode:
            ...
            def compute_delta_CE(node, parent_ce=None):
                # Check that both parent's CE and node.CE are not None
                if parent_ce is not None and node.CE is not None:
                    # Check if both are lists and have the same length
                    if isinstance(parent_ce, list) and isinstance(node.CE, list) and len(parent_ce) == len(node.CE):
                        # Compute element-wise difference between the parent's CE and node's CE
                        delta_CE = [p - c for p, c in zip(parent_ce, node.CE)]
                    else:
                        # If types or lengths do not match, set delta_CE to -1
                        delta_CE = -1
                else:
                    # If either is None, set delta_CE to -1
                    delta_CE = -1

                node_depth = len(node.name)
                with open("/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/target_deltaCE_depth_1.txt", "a") as ce_file:
    ...
    def propose_variable_S(self, node):
        ...
        while GSS_found == 0 and attempts < 8:
            ...
            label_description = self.find_best_matching_label(node.name, new_var.name , "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/GSS2022-varbook.txt")
            ...
            if label_description == "N/A":
                ...
                with open(self.log_pth, "a") as log_file:
                    log_file.write(f"[main {node.name}] [GSS replacement internal steps] : Got an N/A from GSS for the proposed variable. \n")
            ...
            #If we found a match in the GSS dataset
                ...
                try:
                    ...
                    with open(self.log_pth, "a") as log_file:
                        log_file.write(f"[main {node.name}] [GSS replacement internal steps] : Got variable {label_description[1]} from GSS, but failed to limit its domain to less than 7. \n")
                    continue
                if len(domain_dist) > 7:
                    ...
                    with open(self.log_pth, "a") as log_file:
                        log_file.write(f"[main {node.name}] [GSS replacement internal steps] : Got variable {label_description[1]} from GSS, but failed to limit its domain to less than 7. Domain is: {domain_dist}\n")
                    continue

                print(f"debug3: got variable {label_description[1]} from GSS, success!")
                with open(self.log_pth, "a") as log_file:
                    log_file.write(f"[main {node.name}] [GSS replacement internal steps] : Got variable {label_description[1]} from GSS successfully. \n")
                for var in temp_node.unbound:
    ...
    def craft_distribution_S(self, node, var):
        ...
        try:
            ...
            ce_output_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/branching_CE_depth_1.txt"
    ...
    def find_best_matching_label(self, name, query: str, varbook_path: str, top_k: int = 20):
    ...
    def recursive_backtrack(self, node):
        ...
        if not node.children:
            ...
            try:
                ...
                node_depth = len(node.name)
                # Write the (CE loss, depth) pair to file.
                ce_output_path = "/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/target_CE_depth_1.txt"
                with open(ce_output_path, "a") as ce_file:
                    ce_file.write(f"{node_depth}\t{ce_loss}\n")
                node.CE = ce_loss
            except Exception as e:
...
with open("/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/temp.txt", "r") as f:
    lines = f.readlines()
    if not lines:
        raise ValueError("/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/1.txt is empty!")
    for line in lines:
        log_file = "./test_trail_3.txt"
        max_step = 50
        agent = AgentS1("/Users/ruoxiliu/Desktop/Argo_research/reasoning/sense/backtracking/1.json", line, log_file)
        agent.s_agent_entry(max_step, False)
        print("The final calculated target:", agent.s_root.target)
        with open(agent.log_pth, "a") as log_file:
            log_file.write(f"[main 0] [step {max_step}] : Reasoning finished. The final calculated target : {agent.s_root.target} \n")
            log_file.write(f"[main 0] [content] : {json.dumps(agent.s_root.to_json())} \n")