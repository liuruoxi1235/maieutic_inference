...
class AgentS1():
    def __init__(self, config, log_pth='./example_files', ):
        ...
        if self.structured:
            ...
            self.s_root = self.turn_NL_to_S(self.nl_root, "0")
            self.s_root.prob = 1
    ...
    def s_agent_entry(self, max_step = 100, allow_human = False):      
        ...
        if step_mode:
            ...
            while(step < max_step):
                ...
                node = self.queue[0]
                print(f"current queue: {[n.name for n in self.queue]}")
                ...
                #3. craft a distribution for an unbound variable 
                    ...
                    self.queue.popleft()
                    for child in children:
                        print(f"spawned child: {child.name}")
                        self.queue.append(child)
    ...
    #convert an natural language string to an s_node, such as the nl_root to an s_root. gracefully calls the LLM and return the converted node as a Snode object
    def turn_NL_to_S(self, nl_string, name):
        
        with open(self.log_pth, "a") as log_file:
            log_file.write(f"[log] Converting an NL String to an S node. NL String is: {nl_string}\n\n")

        #set up the prompt and the contents
        json_prompt_template = self.structured_prompts["json_prompt"]
        content = {
            "input": nl_string,
            "schema": self.schema
        }

        #do an inference and get the response json content
        s_node = self.single_LLM_call_to_node(json_prompt_template, self.llm, name, content, True)
        self.propose_domain_S(s_node, s_node.target)
